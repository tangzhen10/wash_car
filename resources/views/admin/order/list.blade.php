...
@section('body')
	...
	<table class="table table-border table-bordered table-striped table-hover table-bg table-sort mt-10">
		<thead>
		<tr class="text-c">
			<th width="40">{{trans('common.order_id')}}</th>
			<th width="150">{{trans('common.wash_product')}}</th>
			<th width="40">{{trans('common.address')}}</th>
			<th width="90">{{trans('common.wash_time')}}</th>
			<th width="150">{{trans('common.car_info')}}</th>
			...
			<tr class="text-c">
				...
				<td class="td-manage">
					<a title="{{trans('common.edit')}}" href="javascript:;" class="ml-5"
					   onclick="layer_show($(this).attr('title'),'{{route('memberForm', $row['order_id'])}}','','550')">
